...
class ParsedSectionInstameshNeighbors(ParsedSection):
    neighbors: dict[str, InstameshNeighborEntry] | None = dataclasses.field(
        default=None
    )
    ...
    def parse_section_data(self) -> None:
        ...
        for line in self.text.splitlines():
            ...
            self.neighbors[destination_token] = InstameshNeighborEntry(
                destination=destination_token, cost=int(cost_token)
            )


@dataclasses.dataclass
class ParsedSectionInstameshRoutingTable(ParsedSection):
    entries: dict[str, InstameshRoutingTableEntry] | None = dataclasses.field(
        default=None
    )
    ...
    def parse_section_data(self) -> None:
        ...
        for line in self.text.splitlines():
            if not line or line.startswith("Dest") or line.startswith("----"):
                continue
            destination_token, cost_token, next_hop_token, next_hop_cost_token = (
                line.split()
            )
